    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#files').change(function(){
			  ...
			  {
			   $.ajax({
			    url:"<?php echo base_url(); ?>admin/upload_galery", 
			    method:"POST",
			    data:form_data,
			    contentType:false,
			    cache:false,
			          ...
			          if(idg !='')
		              {
		                 // alert("idg:"+idg);
						  $.ajax({
				              ...
				              success:function(data)
				              {
				                 var message =  "opération reussi avec succès";
								 showSuccessMessage(message);
				                 load_country_data(1);
		 ...
		 $(document).on('submit', '#user_form', function(event){  
               ...
               if(nom != '')
                {
                  ...
                  if (action == 'Edit') {
                        ...
                        $.ajax({  
                             url:"<?php echo base_url() . 'admin/modification_galery_entrep'?>",  
                             method:'POST',  
		  ...
		  $(document).on('click', '.update', function(){  
               var idg = $(this).attr("idg");  
               $.ajax({  
                    url:"<?php echo base_url(); ?>admin/fetch_single_galery_entrep",  
                    method:"POST",  
                    data:{idg:idg},  
                    dataType:"json",  
                    success:function(data)  
