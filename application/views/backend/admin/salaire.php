...
<html class="no-js" lang="en">
		             ...
		             <form method="post" id="user_form" enctype="multipart/form-data">
			            <div class="modal-body">
			            	...
			            	<div class="form-group col-md-12 jf-inputwithicon">
					            <label><i class="fa fa-money"></i> Seil du salaire</label>
					            <input type="text" name="nomsalaire" id="nomsalaire" class="form-control" placeholder="50 - 100 $">
			            ...
			            <div class="modal-footer">
		                    ...
		                    <input type="hidden" name="idsalaire" id="idsalaire" />  
	...
	<script type="text/javascript" language="javascript" >  
     ...
     var dataTable = $('#user_data').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?php echo base_url() . 'admin/fetch_salaire'; ?>",  
                type:"POST"  
           },  
           "columnDefs":[  
                {  
      ...
      $(document).on('submit', '#user_form', function(event){  
           ...
           var nomsalaire = $('#nomsalaire').val(); 
 
           var action = $('#action').val();

           // alert(nomsalaire+" "+action);
           ...
           if(nomsalaire != '')
            {
	            ...
	            if (action =="Add") {
	                ...
	                $.ajax({  
	                     url:"<?php echo base_url() . 'admin/operation_salaire/Add'?>",  
	                     method:'POST',  
		        	...
		        	$.ajax({  
                         url:"<?php echo base_url() . 'admin/modification_salaire'?>",  
                         method:'POST',  
                         data:new FormData(this),  
                         contentType:false,  
                         processData:false,  
      ...
      $(document).on('click', '.update', function(){  
           ...
           $.ajax({  
                url:"<?php echo base_url(); ?>admin/fetch_single_salaire",  
                method:"POST",  
                data:{idsalaire:idsalaire},  
                dataType:"json",  
                success:function(data)  
                {  
                     $('#userModal').modal('show');  
                     $('#nomsalaire').val(data.nomsalaire);
                     $('.modal-title').text("modification de salaire "+data.nomsalaire);  
                     $('#idsalaire').val(idsalaire);  
                     $('#action').val("Edit");  
                }  
           });  
      });
          ...
          if(confirm("Are you sure you want to delete this?"))
	          {
	           		...
	           		$.ajax({
		              url:"<?php echo base_url(); ?>admin/suppression_salaire",
		              method:"POST",
		              data:{idsalaire:idsalaire},
		              success:function(data)
		              {
