...
<script type="text/javascript">
    $(document).ready(function(){
       ...
       function load_country_data(page)
         {
          $.ajax({
           url:"<?php echo base_url(); ?>journaliste/pagination_users_on_to/"+page,
           method:"GET",
           dataType:"json",
            beforeSend:function()
            {
         ...
         {
              $.ajax({
               url:"<?php echo base_url(); ?>journaliste/fetch_search_user_follow_pagination",
               method:"POST",
               data:{query:query},
                beforeSend:function()
                {
         ...
         $(document).on('click', '.voir_chat_message', function(event) {
          ...
          $(document).on('submit', '.my_chat_form', function(event) {
            ...
            var url = "<?php echo(base_url()) ?>journaliste/chat_local_view/"+id_user+"/"+id_recever;
        ...
        function show_message_local_chat(id_user, id_recever){
          ...
          if (id_user !='' && id_recever !='') {
            var url = "<?php echo(base_url()) ?>journaliste/show_message_local_chat/"+id_user+"/"+id_recever;
                $.ajax({  
                     url:url,  
                     method:'POST',  
                     data:{
