	 ...
	 $(document).ready(function(){  
		...
		var  $message = '';
		  ...
		  var dataTable = $('#user_data').DataTable({  
			   ...
			   "order":[],  
			   "ajax":{  
					url:"<?php echo base_url() . 'journaliste/fetch_article_pub_padding_article'; ?>",  
					type:"POST"  
			   },  
			   "columnDefs":[  
					{  
			   ...
			   if(nom != '' && description !='' && idcat !='' && type !='')
				{
				  ...
				  if (action =="Add") {
					  ...
					  $.ajax({  
						   url:"<?php echo base_url() . 'journaliste/operation_padding_article'?>",  
						   method:'POST',  
				  ...
				  if (action == 'Edit') {
						...
						$.ajax({  
							 url:"<?php echo base_url() . 'journaliste/modification_padding_article'?>",  
							 method:'POST',  
							 data:new FormData(this),  
							 contentType:false,  
							 processData:false,  
		  ...
		  $(document).on('click', '.update', function(){  
			   ...
			   $.ajax({  
					url:"<?php echo base_url(); ?>journaliste/fetch_single_padding_article",  
					method:"POST",  
					data:{idart:idart},  
					dataType:"json",  
					success:function(data)  
		  ...
		  $(document).on('click', '.delete', function(){
			  ...
			  if(confirm("Are you sure you want to delete this?"))
			  {
					...
					$.ajax({
					  url:"<?php echo base_url(); ?>journaliste/supression_padding_article",
					  method:"POST",
					  data:{idart:idart},
					  success:function(data)
					  {
		  ...
		  $('.active_all').click(function(event){
			  ...
			  if(checkbox.length > 0)
			  {
				 ...
				 $.ajax({
					url:"<?php echo base_url(); ?>journaliste/validation_article",
					method:"POST",
					data:{
			  ...
			  if(confirm("Etes-vous sûre de vouloir le supprimer?"))
				  {
						...
						if(checkbox.length > 0)
					  {
						 ...
						 // alert("idart:"+checkbox_value);
						 $.ajax({
							url:"<?php echo base_url(); ?>journaliste/validation_article_rejeter",
							method:"POST",
							data:{
							  checkbox_value:checkbox_value
							},
