...
class login extends CI_Controller
{
	...
	public function index(){
		$data["title"] = "Connexion au système Media Devtech media nord kivu";
	...
	public function forgot(){
		$data["title"] = "Avez-vous oublié votre mot de passe au système INPP nord kivu professionnel";  
		...
		$data['pub_publicite']  = $this->crud_model->Select_galery_publicite();
		$data['pub_publicite3']  = $this->crud_model->Select_galery_publicite_lm3();
		$this->load->view('frontend/forgot_view', $data);
	}
	         ...
	         if($result == '')
	         {
				...
				elseif ($this->session->userdata('entreprise_login')) {
			        ...
			        if ($cours->num_rows() > 0) {
			        	...
			        	$insert = array(
							'id_user'	=>	$this->session->userdata('entreprise_login')
						);
						$this->crud_model->insert_online($insert);
			        }
	...
	function register_validation()
  	{
  		  $max = $this->crud_model->get_max_id_user();
		  $ceo = $max + 1;
  	...
  	function register_validation_entreprise()
  	{
          ...
          $avatar = "icone-user.png";
  		  ...
  		  {		
  		  	   ...
  		  	   $debit_event = date('Y-m-d');
               $fin_event =  $this->crud_model->get_expire_day_count($debit_event);
    ...
    function change_secure($param1='', $param2='',$param3='')
    {
        ...
        $this->load->view('frontend/restaure', $data);
    }
    ...
    function recuperaion_password(){
        ...
        if($this->form_validation->run())
        {
            ...
            if($result == '')
            {
                ...
                $subject = "votre mot de passe de recupération au compte system Media";
                $message2 = "
